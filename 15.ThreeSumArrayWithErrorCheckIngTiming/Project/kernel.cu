...
__global__ void sum_array_gpu(int* a, int* b,int *new_, int* c, int size) {
	...
	// do the calculation
	if (gid < size) {
		c[gid] = a[gid] + b[gid] + new_[gid];
	}

}

// for validity checking
void sum_array_cpu(int* a, int* b,int * new_, int* c, int size) {
...
int main() {
	...
	int* h_new = (int*)malloc(bytes_size);
	...
	gpuErrchk(cudaMalloc((int**)&d_new, bytes_size));
	...
	gpuErrchk(cudaMemcpy(d_new, h_new, bytes_size, cudaMemcpyHostToDevice));
	mem_htod_end = clock();
	...
	sum_array_gpu << <grid, block >> > (d_a, d_b,d_new,d_c, size);
	...
	sum_array_cpu(h_a, h_b,h_new, h_c, size);
	cpu_end = clock();
