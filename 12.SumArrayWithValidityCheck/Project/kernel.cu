...
int main() {
	...
	cudaMalloc((int**)&d_a, bytes_size);
	cudaMalloc((int**)&d_b, bytes_size);
	cudaMalloc((int**)&d_c, bytes_size);

	// copy the h_a and h_b pointers to the device
	cudaMemcpy(d_a, h_a, bytes_size, cudaMemcpyHostToDevice);
	cudaMemcpy(d_b, h_b, bytes_size, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(gpu_results, d_c, bytes_size, cudaMemcpyDeviceToHost);
